<template>
  <div class="relative" v-auto-animate>
      ...
      <table class="w-full text-left table-auto min-w-max text-slate-800">
        <thead>
          <tr class="text-slate-500 border-b border-slate-300 bg-slate-50">
            <th class="p-4">
              <p class="text-sm leading-none font-normal">Имя задачи</p>
            </th>
            <th class="p-4">
              <p class="text-sm leading-none font-normal">URL</p>
            </th>
            <th class="p-4">
              <p class="text-sm leading-none font-normal">Дата создания</p>
            </th>
            <th class="p-4">
              <p class="text-sm leading-none font-normal">Действия</p>
            </th>
          </tr>
        </thead>
        <tbody>
    ...
    <div class="logs" v-if="selectedTaskId" v-auto-animate>
      <div
        class="btn-close cursor-pointer hover:scale-110 transition-all"
        @click="closeLogs"
      >
        <svg
      ...
      <div class="logs-body">
        <div class="logs-table" v-if="formStore.taskLogs[selectedTaskId]?.length">
          <h3 class="text-center font-bold text-slate-400">
            Логи задачи {{ selectedTaskId }}
          </h3>
          <table class="w-full text-left table-auto min-w-max text-slate-800">
            <thead>
        ...
        <div class="text-center text-slate-500" v-else>
          У вас пока нет логов по задаче
        </div>
      </div>
    </div>
  </div>
